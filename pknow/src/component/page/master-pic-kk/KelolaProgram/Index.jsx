...
import AppContext_test2 from "./TestContext";
...
export default function MasterProsesIndex({ onChangePage, withID }) {
  ...
  const fetchDataKategori = async (retries = 3, delay = 1000) => {
    for (let i = 0; i < retries; i++) {
      try {
        const data = await UseFetch(API_LINK + "Program/GetKategoriKKById", { id: withID });
    ...
    isLoading ? (
      ...
      <>
        <div className="">
          ...
          <h1>{listKategori.find((item) => item.value === withID)?.label || ""}</h1>
          <p>
          {listKategori.find((item) => item.value === withID)?.deskripsi || ""}
          </p>
